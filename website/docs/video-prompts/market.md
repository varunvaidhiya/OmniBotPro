# OhhO Market — Higgsfield Video Prompt

- **Product:** OhhO Market — *Download a skill. Or sell one.*
...
- **What it does:** A cross-brand marketplace for trained robot skills — verified, signed, tagged by robot + task + success rate, one-click deploy via Serve.
...
SHOT 1/6 — 15 s — Browsing: Hands hold a tablet in the lab, a soft defocused grid of cards glowing on it; behind in the bokeh, a robot waits under cyan practicals. A slow slider frames hands, glass and machine.

SHOT 2/6 — 15 s — The tap: A thumb scrolls, pauses on one card, taps once. Behind, a cyan ripple runs the robot's status ring nose-to-tail — a skill arriving. A rack focus trades tablet for robot.

SHOT 3/6 — 15 s — New trick: The robot immediately performs a skill it has never shown — a precise pick-and-place with a confident rhythm. The camera tracks the motion low and close.

SHOT 4/6 — 15 s — Any body: Wide. Across the room a quadruped's ring ripples the same cyan — and it performs the same handling task its own way, on legs instead of wheels. One skill, two bodies.

SHOT 5/6 — 15 s — Publishing: Reverse flow: the wheeled robot demonstrates a polished move of its own; the hand taps the tablet and it blinks a soft confirm — a skill leaving for the world. Slow push-in on the tablet glow.

SHOT 6/6 — 15 s — Hero: Both robots poised mid-pose, the tablet resting in foreground bokeh, everything steady cyan. A slow fifteen-degree orbit. Upper third clean dark negative space for a title. Hold.
```
...
```
SCENE — OhhO Market (one continuous 15 s shot): A thumb taps one card on a defocused tablet and a cyan ripple runs a robot's ring — it instantly performs a skill it never had, then a quadruped across the room ripples the same cyan and does the task its own way — ending in a slow orbit on both robots with the tablet glowing in the foreground and clean space up top.
```
...
- **Product name:** OhhO Market · **Tagline:** Download a skill. Or sell one.
- **CTA:** `Open the marketplace →` · end on `The Open Robotics Platform`
