...
paste). Tuned for the **trial's unlimited tier: Seedance 2.0 at 720p, 8 s per
shot.** Each `SHOT` block below is **one 8-second generation** — copy the whole
fenced block, paste it, generate, then move to the next shot. Assemble the shots
in order in any editor.

> On a paid plan the same shots go to **15 s / 1080p** — the canonical storyboards
> in `demo.md`, `ohho-os.md` and `README.md` describe that longer/higher-res
> version. This sheet is the trial-optimized cut.
...
| Setting | Value | Why |
|---|---|---|
| **Model** | **Seedance 2.0** | Unlimited on your trial; best multi-shot continuity + native audio. |
| **Duration** | **8 s** (the trial-unlimited maximum) | The unlimited tier caps single shots at 8 s. Total film length comes from sequencing the shots below. |
| **Aspect ratio** | **16:9** | Landscape for the product-page player. |
| **Resolution** | **720p** (the trial-unlimited maximum) | The unlimited tier caps at 720p — fine for web autoplay. Upscale later if needed (see "Going bigger"). |
...
> **Why the shots are written tight:** at 8 s each shot holds **one** dominant
> action and **one** camera move — that is what reads cleanly at this length.
> Don't add extra beats to a shot; add another shot instead.
...
Model **Nano Banana Pro** (also unlimited on your trial), 16:9 (drop to 1K if 2K
isn't covered on your tier). Reuse the result as the first-frame/reference image on
SHOT 1 and the Hero shots, and as the video `poster`.
...
**8 shots × 8 s = 64 s.** Generate each block below as one clip, in order.
...
SHOT — The desk: A dark robotics lab at night, haze in the air. A desk lamp pools over a mechanical keyboard; hands type and a finger firmly presses the enter key. One slow cinematic push-in onto the key. The terminal screen is a soft defocused cyan glow, cyan LED practicals along the room's edges.
...
SHOT — Ship it: A single pulse of cyan light leaves the desk and runs along the floor cable runs, splitting cleanly three ways into the dark. The camera tracks the travelling light low across the polished concrete in one smooth move.
...
SHOT — Body one — wheels: A real wheeled mecanum-drive mobile base wakes, its status ring snapping cyan, and glides one smooth curve across the floor. A low tracking shot rides alongside, rubber wheels catching the light.
...
SHOT — Body two — the arm: A six-axis robot arm wakes with the identical cyan status ring and traces one smooth arc in the air above its bench — the same behavior, a completely different body. A matched slow tracking move, same speed as the previous shot.
...
SHOT — Body three — legs: A four-legged quadruped robot wakes, ring cyan, and walks one smooth curved path across the floor — a third body, the same shared behavior. A low tracking move alongside, matching the previous shots' rhythm.
...
SHOT — Record, train: A teleop bench: hands drive a leader arm while its follower twin mirrors the move under synced record lights; behind them a GPU server rack breathes under load in the dark. One slow dolly binds recording to training.
...
SHOT — Serve, compound: The wheeled robot repeats its pick-and-place noticeably faster and cleaner; beside it a small drone spools up, ring snapping the same cyan — a fourth body joining. A wide, slow push-in as the fleet grows.
...
SHOT — Hero: All four robots — wheeled base, six-axis arm, quadruped and drone — arranged around the desk where it began, status rings breathing cyan in unison, the terminal still glowing. A slow rising orbit pulls up through the haze and holds. Upper third clean dark negative space for the wordmark.
...
**6 shots × 8 s = 48 s.** Generate each block below as one clip, in order.
...
SHOT — Four bodies: Four dark, powered-down robots in a row — a wheeled mobile base, a desk-mounted six-axis arm, a four-legged quadruped, and a drone on its pad — with one small identical compute module in the foreground on the bench. One slow motorized slider along the line-up in deep violet light, haze in the beams.
...
SHOT — One engine: Macro shot. Hands slot the small compute module into the wheeled base's chassis bay; it clicks home and the robot wakes, a violet status ring lighting up. One slow push-in on the machine coming alive.
...
SHOT — Same engine: Identical compute modules click into the arm, then the quadruped, then the drone, one after another — each robot waking with the same violet status ring. The camera tracks smoothly down the line as the lights come on.
...
SHOT — One behavior: Wide shot. All four robots move at once — the same single motion in four dialects: the base glides, the arm arcs, the quadruped walks, the drone flies the same line. Deliberate unison, violet rings breathing together.
...
SHOT — The stack: A defocused monitor beside the robots shows one soft glowing node-graph. The camera racks focus from the glass to the four synced machines beyond it — one runtime, four bodies — in a single slow arcing move.
...
SHOT — Hero: The four robots hold formation, status rings breathing violet in unison, the compute module's identical siblings glinting on the bench in the foreground. A slow rising orbit pulls up through the haze and holds. Upper third clean dark negative space for the title.
...
- **Length:** 8 s is the trial-unlimited per-shot ceiling, so the demo (~64 s) and
  OS (~48 s) films get their length from *sequencing the shots above*. To stretch
  further, run any shot (especially a Hero hold) through Higgsfield's **AI Video
  Extender** to lengthen or loop it seamlessly — every shot ends on a deliberate
  hold for exactly this.
- **Resolution:** generate at 720p now; if you want a crisper master later, run the
  finished cut (or individual shots) through Higgsfield's **video upscaler** to 2K/4K,
  or re-generate at 1080p once you're on a paid plan.
- **More beats:** to enrich a film, add another 8 s shot rather than lengthening an
  existing one — duplicate a block, change only the `SHOT —` action line, keep the
  STYLE and ACCENT identical.
