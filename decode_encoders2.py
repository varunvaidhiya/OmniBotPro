...
import sys, time, struct, serial, statistics, collections
...
def _ck(pkt):  return (sum(pkt) + (257 - DEVICE_ID)) & 0xFF
...
FIELDS = [(f"b{i*2:02d}", i*2) for i in range(9)]  # 9 int16 fields in 18-byte payload
    ...
    for label, offset in FIELDS:
        ...
        for a, b in zip(pkt0e_list, pkt0e_list[1:]):
            if len(a) >= offset+2 and len(b) >= offset+2:
                ...
                deltas.append(d)
        if deltas:
            result[label] = (statistics.mean(deltas),
...
def run_test(ser, label, vx, vy, vz, speed_s=2.5, settle_s=2.0):
    ...
    for _ in range(5):
        ser.write(PKT_STOP); time.sleep(0.02)
    time.sleep(settle_s)

    ds = delta_stats(pkt0e)
    vfb = vel_stats(pkts_all)
    print(f"\n  ── {label}  (0x0E pkts={len(pkt0e)})  vel_feedback_vx≈{vfb:.3f} m/s ──" if vfb is not None
          else f"\n  ── {label}  (0x0E pkts={len(pkt0e)}) ──")
    print(f"  {'Field':6s}  {'mean_delta':>11s}  {'stdev':>8s}  note")
    for label2, offset in FIELDS:
        if label2 not in ds: continue
        m, s, n = ds[label2]
        flag = ""
        if abs(m) > 3*s and abs(m) > 1:    flag = " ← SIGNAL"
        elif abs(m) < 1 and s < 3:          flag = " (noise)"
...
print(f"  Tests run at higher speed so encoder signal > noise.")
print(f"  delta = value[n+1] - value[n]  between consecutive 0x0E pkts")
print(f"{'='*65}")
...
for _ in range(5): ser.write(PKT_STOP); time.sleep(0.02)
time.sleep(0.3)
for _ in range(5): ser.write(PKT_CAR_TYPE); time.sleep(0.05)
time.sleep(0.3)
for _ in range(3): ser.write(PKT_STOP); time.sleep(0.02)
...
print(f"  0x0C vel_feedback at rest: vx≈{vfb_rest:.3f} m/s" if vfb_rest else "  no 0x0C packets")
print(f"  {'Field':6s}  {'mean_delta':>11s}  {'stdev':>8s}")
for lbl, off in FIELDS:
    if lbl not in ds_rest: continue
    m, s, n = ds_rest[lbl]
...
print(f"{'='*65}")
print(f"  {'Field':6s}  {'Rest':>7s}  {'Fwd':>7s}  {'Back':>7s}  {'RotL':>7s}  {'RotR':>7s}  {'StrL':>7s}  {'StrR':>7s}")
print("  " + "-"*72)
...
for lbl, off in FIELDS:
    ...
    print(f"  {lbl:6s}  {r:>7.1f}  {f:>7.1f}  {b:>7.1f}  {rl:>7.1f}  {rr:>7.1f}  {sl:>7.1f}  {sr:>7.1f}  {'  '.join(notes)}")
