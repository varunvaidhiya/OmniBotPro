...
def nav_collision(
    env: ManagerBasedRLEnv, force_threshold: float = 10.0
) -> torch.Tensor:
...
def arm_object_dropped(
    env: ManagerBasedRLEnv, drop_height: float = -0.05
) -> torch.Tensor:
...
def arm_self_collision(
    env: ManagerBasedRLEnv, force_threshold: float = 5.0
) -> torch.Tensor:
