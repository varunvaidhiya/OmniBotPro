...
def nav_action_smoothness(env: ManagerBasedRLEnv) -> torch.Tensor:
    ...
    if not hasattr(env, "_prev_nav_action"):
        env._prev_nav_action = torch.zeros(env.num_envs, 3, device=env.device)
...
def nav_goal_reached_bonus(
    env: ManagerBasedRLEnv, tolerance: float = 0.25
) -> torch.Tensor:
...
def arm_ee_approach(
    env: ManagerBasedRLEnv, sigma: float = 0.05, ee_body_name: str = "arm_gripper_link"
) -> torch.Tensor:
...
def arm_grasp_success(env: ManagerBasedRLEnv) -> torch.Tensor:
    ...
    try:
        ...
        in_contact = (contact_sensor.data.net_forces_w.norm(dim=-1) > 0.5).any(dim=-1)
...
def arm_object_lifted(
    env: ManagerBasedRLEnv, lift_height: float = 0.05
) -> torch.Tensor:
    """
    ...
    try:
        ...
        initial_z = env.extras.get(
            "target_initial_z", torch.zeros(env.num_envs, device=env.device)
        )
...
def arm_object_placed(
    env: ManagerBasedRLEnv, place_tolerance: float = 0.05
) -> torch.Tensor:
    """
    ...
    try:
        ...
        goal_pos = env.extras.get(
            "place_goal_pos", torch.zeros(env.num_envs, 3, device=env.device)
        )
...
def arm_joint_limit_penalty(
    env: ManagerBasedRLEnv, margin: float = 0.1
) -> torch.Tensor:
    """
