...
IMG_WRIST = np.random.randint(0, 256, (480, 640, 3), dtype=np.uint8)   # 640×480 wrist cam
IMG_BEV_800 = np.random.randint(0, 256, (800, 800, 3), dtype=np.uint8)  # 800×800 BEV stitcher output
...
def numpy_to_tensor(img_np: np.ndarray, target_w: int, target_h: int, device) -> "torch.Tensor":
...
def test_bench_normalize():
    ...
    stats = TimingHarness().run(lambda: resized.astype(np.float32) / 255.0, n=10000, warmup=200)
...
def test_bench_both_cameras():
    ...
    print(f"  INFO  Preprocess alone allows up to {fps:.1f} Hz before inference is added.")
    return stats
...
if __name__ == "__main__":
    print("\n" + "=" * 70)
    print(f"OmniBot — Image Preprocessing Benchmarks")
