...
def build_reporters(args: argparse.Namespace) -> List[Reporter]:
    ...
    if args.wandb_project:
        reporters.append(WandbReporter(project=args.wandb_project,
                                       entity=args.wandb_entity,
                                       tags=["benchmark"]))
    if args.prom_port or args.prom_textfile:
        reporters.append(PrometheusReporter(port=args.prom_port or None,
...
def main(argv: List[str] = None) -> int:
    ...
    ap.add_argument("--prom-textfile", default="",
    ...
    print(f"accel   : {system.accelerator_type} ({system.accelerator_name}, "
    ...
    try:
        if "inference" in suites:
            ...
            results.append(InferenceBenchmark(
                policy, task=args.task, n=args.n,
                budget_ms=args.budget_ms, system=system,
            ).run())
        if "training" in suites:
            trainer = TRAINERS.create(args.trainer, **json.loads(args.trainer_kwargs))
            results.append(TrainingBenchmark(
                trainer, episodes_for_training(), system=system,
            ).run())
        ...
        for result in results:
            ...
            for k, v in sorted(result.metrics.items()):
                print(f"   {k:32s} {v:>12.3f}" if isinstance(v, float)
                      else f"   {k:32s} {v}")
