...
class VlaTriggerNode(Node):
    def __init__(self):
        ...
        self._pub_status = self.create_publisher(String, '/perception/vla_trigger_status', 10)
    ...
    def _check(self) -> None:
        ...
        if not (self._tracking_lost and self._loss_start is not None):
            return
        elapsed = (
            self.get_clock().now() - self._loss_start
        ).nanoseconds / 1e9
        if elapsed >= self._loss_thresh:
            self.get_logger().info(
                f'Tracking lost for {elapsed:.1f}s → activating VLA')
            self._activate_vla()
