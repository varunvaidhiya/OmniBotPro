...
class PerceptionNode(Node):
    ...
    def __init__(self):
        ...
        if _ULTRA_OK:
            try:
                ...
                self.get_logger().info(f"Detection model loaded: {p('det_model').value}")
            except Exception as exc:
                self.get_logger().error(f'Detection model load failed: {exc}')
            try:
                self._seg = YOLO(p('seg_model').value)
                self.get_logger().info(f"Segmentation model loaded: {p('seg_model').value}")
            except Exception as exc:
        ...
        # ── Subscriptions ─────────────────────────────────────────────────────
        self.create_subscription(Image, p('front_camera_topic').value, self._front_cb, 1)
        self.create_subscription(Image, p('depth_topic').value, self._depth_cb, 1)
        self.create_subscription(CameraInfo, p('depth_info_topic').value, self._info_cb, 1)
        self.create_subscription(String, '/perception/mode', self._mode_cb, 10)
        self.create_subscription(String, '/perception/target_class', self._target_cb, 10)
        ...
        self._pub_rl_pose = self.create_publisher(PoseStamped, '/rl_arm/target_pose', 10)
        self._pub_rl_det = self.create_publisher(Bool, '/rl_arm/target_detected', 10)
        self._pub_nearest = self.create_publisher(Float32, '/perception/nearest_obstacle_m', 10)
        ...
        self.get_logger().info(f'PerceptionNode ready  mode={self.mode}  hz={1/period:.0f}')
    ...
    def _infer(self) -> None:
        ...
        use_seg = self.mode in (self.MODE_SEGMENT, self.MODE_VLA) and self._seg is not None

        try:
            model = self._seg if use_seg else self._det
            if use_track:
                results = model.track(
                    front, persist=True, conf=self._conf, iou=self._iou,
                    verbose=False, tracker='bytetrack.yaml', device=self._device,
                )
            else:
                results = model.predict(
                    front, conf=self._conf, iou=self._iou,
    ...
    def _depth_at(self, depth: np.ndarray, cx: int, cy: int, r: int = 4) -> float:
        ...
        if not (0 <= cy < h and 0 <= cx < w):
            return 0.0
        patch = depth[max(0, cy - r):min(h, cy + r + 1),
    ...
    def _publish_tracking_state(self, dets: list[dict]) -> None:
        ...
        target_dets = [d for d in dets
                       if d['class_name'].lower() == self._target_class]
        lost = Bool()
    ...
    def _render_masks(self, result, frame: np.ndarray) -> np.ndarray:
        overlay = frame.copy()
        _COLORS = [(0, 230, 0), (230, 0, 0), (0, 0, 230),
