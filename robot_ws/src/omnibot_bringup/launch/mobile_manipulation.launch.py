...
def generate_launch_description():
    ...
    # ------------------------------------------------------------------
    # Base cameras (4x USB, MJPEG to stay within USB bandwidth)
    # Device assignment verified via v4l2-ctl --list-devices on this machine:
    #   front  → /dev/video1  (usb-xhci-hcd.1-1.2)
    #   rear   → /dev/video3  (usb-xhci-hcd.1-1.1.3)
    #   left   → /dev/video5  (usb-xhci-hcd.1-1.3)
    #   right  → /dev/video7  (usb-xhci-hcd.1-1.4)
    # Each USB camera exposes two /dev/videoN nodes; the first (odd) is the
    # capture device, the second (even) is the metadata node.
    # Reassign the labels below if physical placement differs.
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Wrist camera — /dev/video11 (usb-xhci-hcd.1-1.1.2)
    # ------------------------------------------------------------------
