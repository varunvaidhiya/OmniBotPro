...
class RLNavNode(Node):
    ...
    def __init__(self):
        ...
        self.create_subscription(
            PointCloud2, "/camera/depth/points", self._cloud_cb, 10
        )
    ...
    def _goal_cb(self, msg: PoseStamped) -> None:
        self._goal = msg
        self.get_logger().info(
            f"[RLNav] New goal: ({msg.pose.position.x:.2f}, {msg.pose.position.y:.2f})"
        )
    ...
    def _cloud_cb(self, msg: PointCloud2) -> None:
        ...
        try:
            ...
            self.get_logger().warn(
                f"Lidar sector computation failed: {exc}", throttle_duration_sec=5.0
            )
    ...
    def _inference_step(self) -> None:
        ...
        if self._session is not None:
            try:
                ...
                self.get_logger().error(
                    f"Policy inference error: {exc}", throttle_duration_sec=5.0
                )
        ...
        self._cmd_vel_accum[0:2] = np.clip(
            self._cmd_vel_accum[0:2], -self._max_lin, self._max_lin
        )
        self._cmd_vel_accum[2] = np.clip(
            self._cmd_vel_accum[2], -self._max_ang, self._max_ang
        )
